    ...
    public partial class VoxelPlayFirstPersonController : VoxelPlayCharacterControllerBase
    {
        ...
        void LateUpdate ()
        {
            ...
            if (env.cameraHasMoved || forceUpdateCrosshair) {
                ...
                crosshairOnBlock = env.RayCast (ray, out _crosshairHitInfo, hitRange, colliderTypes: ColliderTypes.IgnorePlayer) && _crosshairHitInfo.voxelIndex >= 0;
                if (changeOnBlock) {
                    if (crosshairOnBlock) {
                        ...
                        if (d < -0.2f && switchingLapsed >= 1f) {
                            crosshair.position = _crosshairHitInfo.point;
                            crosshair.LookAt (_crosshairHitInfo.point + _crosshairHitInfo.normal);
                        } else {
                            crosshair.localRotation = Misc.quaternionZero;
                        }
