...
namespace VoxelPlay
{
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        public float GetHeight (Vector3 position)
        {
            ...
            if (!RayCastFast (new Vector3 (position.x, maxAltitude, position.z), Misc.vector3down, out hitInfo, maxDistance, false, 0, ColliderTypes.OnlyVoxels)) {
        ...
        public bool GetVoxelIndex (Vector3 position, out VoxelChunk chunk, out int px, out int py, out int pz, bool createChunkIfNotExists = true)
        {
            int chunkX, chunkY, chunkZ;
            FastMath.FloorToInt (position.x / CHUNK_SIZE, position.y / CHUNK_SIZE, position.z / CHUNK_SIZE, out chunkX, out chunkY, out chunkZ);
            if (GetChunkFast (chunkX, chunkY, chunkZ, out chunk, createChunkIfNotExists)) {
        ...
        /// <param name="position">Position.</param>
        public Voxel GetVoxelUnder (Vector3 position, bool includeWater = false, ColliderTypes colliderTypes = ColliderTypes.AnyCollider)
        {
            VoxelHitInfo hitinfo;
        ...
        /// <returns>The voxel under.</returns>
        /// <param name="position">Position.</param>
        public VoxelIndex GetVoxelUnderIndex (Vector3 position, bool includeWater = false, ColliderTypes colliderTypes = ColliderTypes.AnyCollider)
        {
            VoxelIndex index = new VoxelIndex ();
            VoxelHitInfo hitinfo;
            byte minOpaque = includeWater ? (byte)255 : (byte)0;
        ...
        /// <param name="voxelType">Voxel type.</param>
        /// <param name="modifiedChunks">Optionally return the list of modified chunks</param>
        public void VoxelPlace (Vector3 boxMin, Vector3 boxMax, VoxelDefinition voxelType, List<VoxelChunk> modifiedChunks = null)
        {
        ...
        public bool VoxelHighlight (VoxelHitInfo hitInfo, Color color, float edgeWidth = 0f)
        {
            ...
            if (voxelHighlightGO == null) {
                ...
                voxelHighlight = voxelHighlightGO.GetComponent<VoxelHighlight> () ?? voxelHighlightGO.AddComponent<VoxelHighlight> ();
            } else {
        ...
        public bool VoxelDamage (Vector3 voxelPosition, Vector3 hitPoint, Vector3 normal, int damage, bool addParticles = false, bool playSound = false)
        {
